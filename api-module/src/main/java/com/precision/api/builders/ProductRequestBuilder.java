...
public class ProductRequestBuilder {
    ...
    public static Response getAllProducts() {
        return given().spec(ApiClient.getBaseSpec()).when().get("/api/productsList");
    }

    public static Response searchProduct(String productName) {
        return given().spec(ApiClient.getBaseSpec()).formParam("search_product", productName).when().post("/api/searchProduct");
    }
}
